...
const OUT=process.env.SHOTS_DIR ?? '/tmp/ff-my'; mkdirSync(OUT,{recursive:true});
...
for (const sig of ['exit','SIGINT','SIGTERM','uncaughtException']) {
  process.on(sig, () => { try { chrome.kill(); } catch {} });
}
...
await send('Page.navigate',{url:(process.env.APP_URL ?? 'http://localhost:5173')+'/'});
...
check('gate is in Burmese', await js(`document.body.innerText.includes('အဖွဲ့ကုဒ်')`), (await js('document.body.innerText')).slice(0,90).replace(/\n/g,' | '));
await shoot('01-gate-my');

console.log('\nsign in');
const INVITE_CODE = process.env.INVITE_CODE ?? 'futsal-dev';
await js(`(()=>{const f=document.querySelector('md-outlined-text-field');f.value=${JSON.stringify(INVITE_CODE)};f.dispatchEvent(new Event('input',{bubbles:true,composed:true}))})()`);
await sleep(300);
await js(`document.querySelector('md-filled-button').click()`);
await waitFor(`document.body.innerText.includes('သင်က ဘယ်သူလဲ')`,'name picker in Burmese');
await js(`[...document.querySelectorAll('md-outlined-button')][0].click()`);
